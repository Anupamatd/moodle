...
class xmldb_field extends xmldb_object {
    ...
    public function setDefault($default) {
        ...
        if (in_array($this->type, [XMLDB_TYPE_TEXT, XMLDB_TYPE_BINARY, XMLDB_TYPE_JSON]) && $default !== null) {
            $this->errormsg = 'XMLDB has detected one TEXT/BINARY/JSON column (' . $this->name .
                ") with some DEFAULT defined. This type of columns cannot have any default value. " .
    ...
    public function arr2xmldb_field($xmlarr) {
        ...
        if (isset($xmlarr['@']['LENGTH'])) {
            ...
            if (in_array($this->type, [XMLDB_TYPE_TEXT, XMLDB_TYPE_BINARY, XMLDB_TYPE_JSON])) {
                $length = null;
            }
    ...
    public function validateDefinition(?xmldb_table $xmldb_table=null) {
        ...
        switch ($this->getType()) {
            ...
            case XMLDB_TYPE_JSON:
                if (
                    $this->getLength() !== null || $this->getDecimals() !== null ||
                    $this->getSequence() || $this->getDefault() !== null
                ) {
                    // phpcs:ignore moodle.NamingConventions.ValidVariableName.VariableNameUnderscore -- Existing API parameter name.
